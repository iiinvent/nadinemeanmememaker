...
export async function generateMemeText(theme: string): Promise<{ topText: string; bottomText: string }> {
  try {
    ...
    const systemPrompt = `
You are a witty meme creator who specializes in viral, relatable, and clever memes. 
You understand internet culture, current trends, and how to make people laugh. 
Your memes should be punchy, memorable, and follow classic meme formats.
`.trim();

const userPrompt = (theme: string) => `
Create a hilarious meme about: ${theme}
